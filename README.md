...
| --- | --- |
| Public release | `v0.6.0` via [publication/manifest.json](publication/manifest.json) |
...
| Sealed-source census | 1550 tracked files classified; zero unclassified = `True` |
| Midnight readiness | full universal path for `0.6.0`: status=`ready`, local_operator=`True`, live_submit=`False` via [midnight/readiness.json](midnight/readiness.json) |
| EVM readiness | secondary deploy-capable lane for `0.6.0`: status=`secondary-ready` via [evm/readiness.json](evm/readiness.json) |
...
| Sealed-source census | Opaque private-file census summarized publicly | **1550 files; zero unclassified = True** |
| Binary integrity | Published release manifest `binary-manifest/v0.6.0/manifest.json` | **SHA-256 verified for `aarch64-apple-darwin`** |
| Midnight operator path | Full universal path for `0.6.0` | **status=ready local_operator=True live_submit=False** |
| EVM operator path | Secondary deploy-capable lane for `0.6.0` | **status=secondary-ready** |
